...
for y in range(int((3*height)/5)+100,height-50,5):
    cnt = 0
    flag = 0
...
# Assuming midx and midy are already defined

# Fit a line to the points
# coefficients = np.polyfit(midx[::10], midy[::10], 1)
# poly = np.poly1d(coefficients)

# # Generate y values for the line
# y_values = np.linspace(min(midy), max(midy), 1000)

# y_dots = np.linspace(min(midy), max(midy), 10)
# x_dots = poly(y_dots)
# # Calculate corresponding x values using the inverse of the fitted line
# x_values = poly(y_values)
# # Convert the line coordinates to integers
# x_values_int = x_values.astype(int)
# y_values_int = y_values.astype(int)
# x_dots = x_dots.astype(int)
# y_dots = y_dots.astype(int)
# # Draw the line on the image
# for i in range(len(x_values_int) - 1):
#     cv2.line(image_out, (x_values_int[i], y_values_int[i]), (x_values_int[i+1], y_values_int[i+1]), (255, 0, 0), thickness=8)
# for i in range(len(x_dots)):
#     cv2.circle(image_out, (x_dots[i],y_dots[i]), 10,(255, 0, 0), -1)    
# for x, y in zip(midx[::10], midy[::10]):
#     cv2.circle(image_out, (int(x), int(y)), 10,(0, 0, 255), -1)  # Draw filled circle
# Convert midx and midy to a list of tuples
points = [(int(x), int(y)) for x, y in zip(midx[::10], midy[::10])]

# Draw filled circles at every 10th point
for x, y in points:
    cv2.circle(image_out, (x, y), 10, (0, 0, 255), -1)  # Draw filled circle

# Draw a curved line connecting the points
cv2.polylines(image_out, [np.array(points)], isClosed=False, color=(255, 0, 0), thickness=5,lineType=cv2.LINE_AA)

# Save the modified image
cv2.imwrite('image_with_line.jpg', image_out)
#------------------------------------------------------
...
# cv2.imwrite("way_points_dots.png", image_with_dots)
#------------------------------------------------------
